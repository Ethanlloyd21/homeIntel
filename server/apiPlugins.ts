...
import { trafficTilesProxy } from './trafficTilesProxy.ts'
import { gasPriceProxy } from './gasPriceProxy.ts'
import { censusProxy } from './censusProxy.ts'
...
const fbiCrimeProxy = (apiKey: string): Plugin => {
  const handleRequest = async (
    ...
    try {
      ...
      if (!result.ok) throw new Error('FBI crime data unavailable')
...
const collegeScorecardProxy = (apiKey: string): Plugin => {
  ...
  const handleRequest = async (
    ...
    try {
      ...
      if (!result.ok) throw new Error('College Scorecard data unavailable')
      const body = await result.text()
      response.statusCode = result.status
      if (result.ok)
        cache.set(state, {
...
const loadStateSchools = (fips: string) => {
  ...
  const request = (async () => {
    ...
    let page = 0
    while (next && page < 3) {
      const result = await upstreamFetch(next, {
        headers: {
...
const placeSearchProxy = (loadPlaces?: PlaceSearchLoader): Plugin => {
  const handleRequest = async (
    request: IncomingMessage,
    response: ServerResponse,
    ...
    try {
      const run = loadPlaces
        ? loadPlaces(params)
        : placeSearchChain.then(async () => {
            const upstream = await timedFetch(
...
export const createApiPlugins = (
  env: Record<string, string | undefined>,
  loadPlaces?: PlaceSearchLoader,
): Plugin[] => {
  const dataGovKey = env.DATA_GOV_API_KEY || ''
  // Legacy local key is read on the server only; it is never used by the frontend.
  const censusKey = env.CENSUS_API_KEY || env.VITE_CENSUS_API_KEY || ''
  return [
    censusProxy(censusKey),
    fbiCrimeProxy(dataGovKey),
    collegeScorecardProxy(dataGovKey),
    nearbySchoolsProxy(),
    currentEconomyProxy(censusKey, env.BEA_API_KEY || ''),
    majorEmployersProxy(),
    federalContractorsProxy(),
    majorHospitalsProxy(),
    trafficCommuteProxy(env.TOMTOM_API_KEY || ''),
    trafficTilesProxy(env.TOMTOM_API_KEY || '', env.ARCGIS_API_KEY || ''),
    gasPriceProxy(env.EIA_API_KEY || dataGovKey),
    placeSearchProxy(loadPlaces),
  ]
}
